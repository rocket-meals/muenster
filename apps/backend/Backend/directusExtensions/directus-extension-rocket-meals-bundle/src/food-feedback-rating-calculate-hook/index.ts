...
export default defineHook(async ({ action, filter }, apiContext) => {
  ...
  action(collection + '.items.update', async meta => {
    ...
    let food_ids =
      await foodRatingCalculator.getFoodIdsFromFoodFeedbackIds(
        food_feedbacks_ids
      );
    let ignore_food_feedback_ids: string[] = []; // we do not want to ignore any food_feedbacks since we just created them
    foodRatingCalculator.recalculateFoodIdsRatingsNonBlocking(
      food_ids,
      ignore_food_feedback_ids
    );
    // {
  ...
  filter(
    ...
    async (
      ...
      if (
        !food_feedbacks_ids ||
        !Array.isArray(food_feedbacks_ids) ||
        food_feedbacks_ids.length === 0
      ) {
        // do nothing
      } else {
        let food_ids =
